...
docker build -t switchaphon/multi-client:$SHA -f ./client/Dockerfile ./client
docker build -t switchaphon/multi-server:$SHA -f ./server/Dockerfile ./server
docker build -t switchaphon/multi-worker:$SHA -f ./worker/Dockerfile ./worker
